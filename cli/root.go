// Package cli assembles the orcid command tree from the orcid
...
// NewApp assembles the kit application from the orcid domain. The
...
//
// To add a command, declare it in orcid/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := orcid.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(orcid.Domain{}).Register(app)

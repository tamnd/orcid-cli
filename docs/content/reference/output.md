...
describes what users get. Pick a format with `-o`, or let orcid choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
orcid <command> -o table   # aligned columns for reading
orcid <command> -o jsonl   # one JSON object per line, for piping
orcid <command> -o json    # a single JSON array
orcid <command> -o csv     # spreadsheet friendly
orcid <command> -o tsv     # tab-separated
orcid <command> -o url     # just the URL column
orcid <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
orcid <command> --fields id,title,url
```
...
```bash
orcid <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
orcid <command>            # a table, because this is a terminal
orcid <command> | wc -l    # JSONL, because this is a pipe
```
